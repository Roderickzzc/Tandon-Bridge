...
/*void Ant::breed() {
	...
	if (breedTicks == ANTBREED) {
		...
		if ((y > 0) && (wPtr->getThing(x, y - 1) == nullptr)) {
			Direction.push_back(0);
		}
		if ((y < WorldSize - 1) && (wPtr->getThing(x, y + 1) == nullptr)) {
			Direction.push_back(1);
		}
		if ((x > 0) && (wPtr->getThing(x-1, y) == nullptr)) {
			Direction.push_back(2);
		}
		if ((x < WorldSize-1) && (wPtr->getThing(x + 1, y) == nullptr)) {		
			Direction.push_back(3);
		}

		int whichDirection = rand() % Direction.size();

		if (Direction[whichDirection] == 0) {
			Ant* newAnt = new Ant(wPtr, x, y - 1);
		}
		else if (Direction[whichDirection] == 1) {
			Ant* newAnt = new Ant(wPtr, x, y + 1);
		}
		else if (Direction[whichDirection] == 2) {
			Ant* newAnt = new Ant(wPtr, x - 1, y);
		}
		else if (Direction[whichDirection] == 3) {
			Ant* newAnt = new Ant(wPtr, x + 1, y);
		}
	}
}*/
